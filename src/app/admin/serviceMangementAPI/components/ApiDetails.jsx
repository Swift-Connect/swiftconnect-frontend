...
const ApiDetails = ({ title, setCard, path }) => {
  ...
  const [showAddExistingPlansModal, setShowAddExistingPlansModal] =
    useState(false);
  ...
  const fetchApis = async () => {
    ...
    try {
      ...
      if (sortedApis.length > 0) {
        if (
          !selectedApi ||
          !sortedApis.find((api) => api.id === selectedApi.id)
        ) {
          console.log("Setting new selected API:", sortedApis[0]);
          setSelectedApi(sortedApis[0]);
        } else {
          // Update the selected API with the latest data
          const updatedSelectedApi = sortedApis.find(
            (api) => api.id === selectedApi.id
          );
          if (updatedSelectedApi) {
            console.log(
              "Updating selected API with latest data:",
              updatedSelectedApi
            );
  ...
  const handleSelectAll = async () => {
    ...
    try {
      ...
      const existingPlanIds = selectedApi.plans.map((plan) => plan.id);
      const newPlanIds =
        existingPlanIds.length === allPlanIds.length ? [] : allPlanIds;
  ...
  const handleSavePlanEdit = async () => {
    ...
    try {
      ...
      const currentPlanIds = Array.isArray(selectedApi.plan_ids)
        ? selectedApi.plan_ids
        : [];
    ...
    if (existingPlansSearchTerm) {
      filtered = filtered.filter(
        (plan) =>
          plan.name
            .toLowerCase()
            .includes(existingPlansSearchTerm.toLowerCase()) ||
