...
const Dashboard = () => {
  ...
  const [activeTabTransactions, setActiveTabTransactions] = useState('All Transactions')
  ...
  const filteredTransactionData = allTransactionData.filter(tx => {
    const matchesFilter = transactionFilter === 'All' || 
      (transactionFilter === 'Success' && tx.status.toLowerCase() === 'completed') ||
    ...
    const matchesDate = !dateRange.start || !dateRange.end ||
      (new Date(tx.date) >= new Date(dateRange.start) && new Date(tx.date) <= new Date(dateRange.end))
  ...
  const totalPagesTrx = Math.ceil(filteredTransactionData.length / itemsPerPage)
  ...
  const fetchKYC = useCallback(async () => {
    ...
    try {
      ...
      // Sort by most recent
      processedUserKYCData.sort((a, b) => new Date(b.created_at) - new Date(a.created_at))
      setUsersKYCPendingData(processedUserKYCData)
    } catch (error) {
      handleApiError(error, 'Failed to fetch KYC data')
    } finally {
  ...
  const fetchTransactions = useCallback(async () => {
    ...
    try {
      ...
      const processedDataTrx = allTransactions.map(tx => ({
        ...
        date: tx.created_at ? new Date(tx.created_at).toLocaleDateString('en-GB') : 'N/A',
      ...
      // Sort by most recent
      processedDataTrx.sort((a, b) => new Date(b.created_at) - new Date(a.created_at))
  ...
  useEffect(() => {
    ...
    fetchData()
  }, [token, fetchUsers, fetchKYC, fetchTransactions])

  useEffect(() => {
    if (userssData.length || allTransactionData.length || usersKYCPendingData.length) {
      fetchStats()
    }
        ...
        <div className='grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8'>
          ...
          <div className='bg-white p-6 rounded-lg shadow-sm border'>
            ...
            <div className="mb-4">
              ...
              <p className='text-green-600 text-sm font-medium'>Total Income +2.45%</p>
            </div>
          ...
          <div className='bg-white p-6 rounded-lg shadow-sm border'>
            ...
            <div className="mb-4">
              ...
              <p className='text-green-600 text-sm font-medium'>Visitors +2.45%</p>
            </div>
          ...
          {/* User Breakdown */}
          <div className='bg-white p-6 rounded-lg shadow-sm border'>
            <h3 className='text-lg font-semibold text-gray-800 mb-4'>Users Breakdown</h3>
