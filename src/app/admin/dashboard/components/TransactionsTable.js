...
const TransactionsTable = ({ setShowEdit, handleDelete }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const processedData = validTransactions.map((tx) => {
  ...
  return (
    <>
      {isLoading ? (
        <div className="text-center py-8">Loading transactions, please wait...</div>
      ) : data.length === 0 ? (
